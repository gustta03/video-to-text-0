...
describe('BcryptHashAdapter', () => {
  ...
  describe('hash', () => {
    ...
    it('should throw an error if hash generation fails', async () => {
      const plaintext = 'password123'
      const invalidSalt = -1
      const bcryptHashAdapter = new BcryptHashAdapter(invalidSalt)

      await expect(bcryptHashAdapter.hash(plaintext)).rejects.toThrow()
    })
  })
  ...
  describe('compare', () => {
    ...
    it('should throw an error if comparison fails', async () => {
      ...
      await expect(bcryptHashAdapter.compare(plaintext, invalidHash)).rejects.toThrow()
    })
  })
})
